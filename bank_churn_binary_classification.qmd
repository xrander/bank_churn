...
## Random Forest
The right values to use, when training a single model is unknown, but we can train a group and see which turns out best. Our training data is large, more than 132926 rows and 11 columns, a sample of this data should be collected, and be used to train the random forest model. Parallelization will also be used here to speed up to processing time. 
```{r}
...
From the plot above, it seems low mtry (from 3 to 5) are good and high min_n (25 to 35) are good. We should a set range between this values using grid_regular for tuning one more time.
```{r}
rf_grid <- 
  grid_regular(
    min_n(range = c(25, 35)),
