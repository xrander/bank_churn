...
train_data_split <- initial_validation_split(train_data_preproc, prop = c(.6, .2), strata = exited)

training_data <- training(train_data_split)
validation_data <- validation(train_data_split)
testing_data <- testing(train_data_split)
```
...
Logistic regression is having an accuracy of 83% and an area under the curve of 0.82%

```{r}
conf_mat(log_reg_assess, truth = exited, estimate = .pred_class)
```

The model predicts non-event well than event

### Parameter tuning for decision tree and random forest
